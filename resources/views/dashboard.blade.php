@php
$userLevel = null;
if ($user->xp_total >= 0 && $user->xp_total < 5000 ) { $userLevel="Apprenti" ; } elseif ($user->xp_total >= 5000 &&
    $user->xp_total < 10000 ) { $userLevel="Niveau 2" ; } elseif ($user->xp_total >= 10000 && $user->xp_total < 30000 )
            { $userLevel="Niveau 3" ; } elseif ($user->xp_total >= 30000 && $user->xp_total < 50000 ) {
                $userLevel="Niveau 4" ; } elseif ($user->xp_total >= 50000) {
                $userLevel = "God";
                }
                @endphp
                <x-app-layout>
                        ...
                        <div class="max-w-7xl mx-auto sm:px-8 lg:px-8 space-y-8">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                ...
                                <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
                                    ...
                                    <p class="text-3xl font-black text-orange-500">{{ $user->current_streak }} jours 🔥
                                    </p>
                                </div>
                                ...
                                <div class="flex flex-col md:flex-row items-center gap-8">
                                    <div class="relative h-32 w-32 flex items-center justify-center">
                                        <svg class="h-full w-full" viewBox="0 0 36 36">
                                            <path class="text-gray-200" stroke-width="3" stroke="currentColor"
                                                fill="none"
                                        ...
                                        <span
                                            class="absolute text-xl font-black text-gray-800">{{ round($progressPercent) }}%</span>
                                    </div>

                                    <div class="flex-1">
                                        <h3 class="text-2xl font-bold text-gray-900 mb-2">Ta maîtrise des verbes</h3>
                                        <p class="text-gray-600 mb-4">Tu as appris
                                            <strong>{{ $learnedVerbsCount }}</strong> verbes sur
                                            un total de <strong>{{ $totalVerbs }}</strong>. Continue comme ça !
                                        </p>
                                        <a href="{{ route('learn') }}"
                                ...
                                <div class="bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                                    ...
                                    <div class="space-y-4">
                                        <div
                                            ...
                                            <span class="text-indigo-600 font-bold">{{ $user->daily_target }}
                                                verbes</span>
                                        </div>
                                        <p class="text-xs text-gray-400 italic">Modifier l'objectif dans les paramètres
                                            du profil.</p>
                                    </div>
                                </div>

                                <div
                                    class="bg-indigo-900 p-8 rounded-3xl shadow-xl text-white relative overflow-hidden">
                                    <div class="relative z-10">
                                        <h4 class="text-lg font-bold mb-2">Prêt pour un test ?</h4>
                                        <p class="text-indigo-200 mb-6 text-sm">Entraîne-toi sur tous les verbes que tu
                                            as déjà vus pour
