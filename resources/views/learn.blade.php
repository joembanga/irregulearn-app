                ...
                <section class="card-surface rounded-2xl p-6 shadow-lg border border-muted">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div class="hidden sm:flex items-center gap-3">
                            <a href="{{ route('verbs.index') }}" class="text-sm text-muted hover:text-accent">Voir la liste des verbes</a>
                        </div>
                    </div>
                </section>
            @else
                            ...
                            @foreach ($categories as $category)
                                <div
                                    ...
                                    <div class="flex items-start flex-1">
                                        <div @class([
                                            'flex-shrink-0 w-16 h-16 rounded-xl flex items-center justify-center text-surface text-2xl font-bold',
                                        ...
                                        <div class="ml-4 flex-1 flex flex-col">
                                            ...
                                            @if (!$category->is_locked)
                                                <div class="w-full bg-surface rounded-full h-2.5 mb-4 overflow-hidden">
                                                    <div class="bg-primary h-2.5 rounded-full transition-all duration-700" style="width: {{ $category->progress }}%"></div>
                                                </div>
                                            ...
                                            @else
                                                <div class="flex items-center gap-3">
                                                    <div class="flex-1">
                                                        <div class="text-xs font-semibold text-warning uppercase tracking-widest">Verrouillé</div>
                                                        <div class="text-sm text-gray-500 dark:text-gray-400">
                                                            Coût: <span class="font-bold">{{ $category->cout }} XP</span>
                                                        </div>
                                                    </div>
                                                    <div class="flex-shrink-0">
                                                        <button class="px-4 py-2 bg-primary hover:bg-primary/90 text-surface rounded-lg font-semibold">Déverrouiller</button>
                                                    </div>
                                                </div>
                                            @endif
                                        </div>
