...
<x-app-layout>
    <div x-data="{
        ...
        <!-- Hero Header -->
        <div class="bg-linear-to-b from-primary/10 to-transparent pt-12 pb-24 overflow-hidden">
            <div class="max-w-4xl mx-auto px-6 relative z-10">
                <div class="flex flex-col md:flex-row items-start md:items-end justify-between gap-8">
                    <div>
                        <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary text-[10px] font-bold uppercase tracking-widest mb-4">
                            <span>🇬🇧</span> English Irregular Verb
                        </div>
                        <h1 class="text-6xl md:text-8xl font-bold text-body tracking-tighter uppercase mb-4 leading-none">
                            {{ $verb->infinitive }}
                        </h1>
            ...
            <div class="absolute -right-20 -top-20 text-[20rem] font-bold text-primary opacity-[0.03] pointer-events-none select-none">
        ...
        <div class="max-w-4xl mx-auto px-6 -mt-12 space-y-8 relative z-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="card-surface p-8 rounded-[2.5rem] border border-muted shadow-xl group hover:border-primary transition-all duration-500">
                    <div class="flex items-center justify-between mb-6">
                        <span class="text-[10px] font-bold text-muted uppercase tracking-[0.2em]">Past Simple</span>
                    ...
                    <p class="text-4xl font-bold text-body group-hover:text-primary transition-colors">
                ...
                <div class="card-surface p-8 rounded-[2.5rem] border border-muted shadow-xl group hover:border-success transition-all duration-500">
                    <div class="flex items-center justify-between mb-6">
                        <span class="text-[10px] font-bold text-muted uppercase tracking-[0.2em]">Past Participle</span>
                        <button @click="speak('{{ $verb->past_participle }}')" class="p-2 rounded-xl bg-app text-muted hover:text-success transition">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M14,3.23V5.29C16.89,6.15 19,8.83 19,12C19,15.17 16.89,17.85 14,18.71V20.77C18.01,19.86 21,16.28 21,12C21,7.72 18.01,4.14 14,3.23M16.5,12C16.5,10.23 15.5,8.71 14,7.97V16.02C15.5,15.29 16.5,13.77 16.5,12M3,9V15H7L12,20V4L7,9H3Z"/></svg>
                        </button>
                    </div>
                    <p class="text-4xl font-bold text-body group-hover:text-success transition-colors">
            ...
            <!-- Meaning & Description -->
            <div class="card-surface p-8 md:p-12 rounded-[3rem] border border-muted shadow-xl bg-linear-to-br from-muted to-primary/5">
                ...
                <a href="{{ $verb->source_url . '#English' }}" target="_blank"
                    class="px-6 py-3 bg-surface border-2 border-primary/20 text-primary rounded-2xl font-bold text-[10px] hover:bg-primary hover:text-surface transition-all uppercase tracking-[0.2em] shadow-lg shadow-primary/5 active:scale-95">
                    En savoir plus
                </a>
                </div>
            </div>
            ...
            @if($sentences->count() > 0)
                <div class="space-y-4">
                    <h3 class="text-xl font-bold text-body uppercase tracking-tight px-4">Exemples d'utilisation</h3>
                        ...
                        @foreach($sentences as $sentence)
                            <div class="card-surface p-6 rounded-2xl border border-muted flex items-start justify-between gap-4 group hover:bg-primary/5 transition-all">
                                <p class="text-body font-medium leading-relaxed">
                                    {!! str_replace($sentence->missing_word, '<span class="text-primary font-bold underline">' . $sentence->missing_word . '</span>', $sentence->sentence) !!}
                                </p>
