...
<body class="antialiased font-sans text-body scroll-smooth bg-linear-to-b from-surface via-surface to-app/5 selection:bg-primary/20">
    ...
    <main class="relative overflow-hidden">
        <div class="max-w-3xl mx-auto px-6">
            <h1 class="text-3xl font-extrabold text-body mb-4">{{ __('Conditions d\'utilisation') }}</h1>
            <p class="text-gray-700 dark:text-gray-400 mb-6">
                {{ __('Bienvenue sur IrreguLearn. En utilisant notre service, vous acceptez ces conditions. Merci de lire attentivement.') }}
            </p>

            <h2 class="text-xl font-semibold text-body mt-6">{{ __('1. Usage du service') }}</h2>
            <p class="text-gray-700 dark:text-gray-400">
                {{ __('Le service est fourni pour un usage personnel et éducatif. Toute utilisation commerciale non autorisée est interdite.') }}
            </p>

            <h2 class="text-xl font-semibold text-body mt-6">{{ __('2. Comptes') }}</h2>
            <p class="text-gray-700 dark:text-gray-400">
                {{ __('Vous êtes responsable de la sécurité de votre compte. Ne partagez pas vos identifiants.') }}</p>

            <h2 class="text-xl font-semibold text-body mt-6">{{ __('3. Modifications') }}</h2>
            <p class="text-gray-700 dark:text-gray-400">
                {{ __('Nous pouvons mettre à jour ces conditions, nous vous informerons des changements importants.') }}
            </p>

            <p class="text-sm text-gray-500 dark:text-gray-400 mt-8">{{ __('Dernière mise à jour :') }}
                {{ date('Y-m-d') }}</p>
        </div>
    </main>
                ...
                <div>
                    ...
                    <ul class="space-y-4 text-sm font-bold text-body">
                        <li><a href="#" wire:navigate
                                class="hover:text-primary transition-colors">{{ __('Centre d\'aide') }}</a>
                        </li>
                        <li><a href="#" wire:navigate
                                class="hover:text-primary transition-colors">{{ __('Contactez-nous') }}</a>
                        </li>
                        <li><a href="#" wire:navigate
                                class="hover:text-primary transition-colors">{{ __('Confidentialité') }}</a></li>
                    </ul>
                </div>
                <div>
                    ...
                    <div class="flex gap-4">
                        <a href="#"
                            class="w-10 h-10 text-lg flex items-center justify-center hover:scale-[1.02] transition-all">𝕏</a>
                        <a href="#"
                            class="w-10 h-10 flex items-center justify-center hover:scale-[1.02] transition-all">
