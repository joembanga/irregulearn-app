<div class="w-full">
    ...
    <div class="space-y-12 animate-fade-in">
        ...
        @if($verbs->count() > 0)
        <div>
            <h3 class="text-[10px] font-black text-muted uppercase tracking-[0.2em] mb-6 flex items-center gap-3">
                📖 Verbes irréguliers
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div wire:click="selectResult('{{ $verb->infinitive }}', '{{ route('verbs.show', $verb->slug) }}')"
                    ...
                    <div class="flex items-center gap-4">
                        <div class="w-12 h-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-black text-xs">
                            EN
                        </div>
                        <div>
        ...
        <div class="text-center py-20 bg-muted/5 rounded-[3rem] border-4 border-dashed border-muted/20">
            <div class="text-6xl mb-6">🔍</div>
            <h3 class="text-xl font-black text-body uppercase mb-2">Aucun résultat</h3>
    ...
    <div class="mt-8 animate-fade-in-up">
        <div class="flex items-center justify-between mb-8 px-2">
            <h3 class="text-[10px] font-black text-muted uppercase tracking-[0.2em]">Historique récent</h3>
            <button wire:click="clearHistory"
                class="text-[10px] font-black text-danger uppercase tracking-[0.2em] hover:opacity-70 transition-opacity">
                Effacer tout
            </button>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <button wire:click="searchFromHistory('{{ $term }}')"
                ...
                <span class="mr-4 text-muted group-hover:text-primary transition-colors">🕒</span>
                <span class="font-bold text-body tracking-tight group-hover:text-primary transition-colors">{{ $term }}</span>
    ...
    @else
    <div class="text-center py-24 bg-surface rounded-[4rem] border border-muted shadow-2xl relative overflow-hidden group">
        <div class="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent pointer-events-none"></div>
        <div class="relative z-10 flex flex-col items-center">
            <div class="w-24 h-24 rounded-[2rem] bg-primary/10 flex items-center justify-center mb-8 rotate-3 transition-transform group-hover:rotate-0 duration-500">
                <svg class="w-10 h-10 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                        d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path>
                </svg>
            </div>
            <h3 class="text-2xl font-black text-body tracking-tight uppercase mb-2">Commence ton exploration</h3>
