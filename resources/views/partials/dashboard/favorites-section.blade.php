<div
    class="card-surface p-8 md:p-10 rounded-[3rem] border border-muted relative overflow-hidden group/favs shadow-2xl shadow-muted/5">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 relative z-10">
        ...
        <a href="{{ route('verbs.show', $fav->slug ?? '') }}" wire:navigate
            class="group relative p-6 bg-surface rounded-[2rem] border-2 border-muted hover:border-primary hover:-translate-y-2 hover:shadow-[0_20px_50px_rgba(var(--primary-rgb),0.1)] transition-all duration-500 overflow-hidden flex flex-col justify-between min-h-[160px]">
            <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span
                    class="bg-primary/10 text-primary text-[8px] font-black px-2 py-1 rounded-full uppercase tracking-widest">
                    {{ $stats['percentage'] }}%
                </span>
            </div>
            ...
            <div>
                <h4
                    class="text-xl font-black text-body group-hover:text-primary transition-colors tracking-tighter uppercase leading-tight mb-4">
                    {{ $fav->infinitive }}
                </h4>

                <div class="flex flex-wrap gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                    <span
                        class="text-[8px] font-black text-muted uppercase tracking-widest">{{ $fav->past_simple }}</span>
                    <span
                        class="text-[8px] font-black text-muted uppercase tracking-widest">{{ $fav->past_participle }}</span>
                </div>
            </div>
        ...
        @empty
        <div
            class="col-span-full text-center py-6 lg:py-12 bg-muted/5 rounded-[2.5rem] border-2 border-dashed border-muted/30">
    ...
    <!-- Decorative BG -->
    <div
        class="absolute -right-16 -bottom-16 text-[10rem] opacity-[0.03] font-black text-muted/100 rotate-12 pointer-events-none uppercase group-hover/favs:rotate-0 transition-transform duration-1000">
        STAR
    </div>
</div>