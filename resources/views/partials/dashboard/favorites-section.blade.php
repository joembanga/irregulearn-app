<div class="card-surface p-6 md:p-8 rounded-2xl border border-muted relative overflow-hidden group/favs shadow-2xl shadow-muted/5">
    <div class="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-5 relative z-10">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h3 class="text-2xl font-bold text-body flex items-center gap-3 tracking-tighter">
        ...
        <a href="{{ route('verbs.favorites') }}" wire:navigate
            class="px-6 py-3 text-center bg-surface border-2 border-primary/20 text-primary rounded-xl font-bold text-[10px] hover:bg-primary hover:text-surface transition-all uppercase tracking-[0.2em] shadow-lg shadow-primary/5 active:scale-95">
            Gérer ma collection <x-lucide-move-right class="size-3 inline stroke-3" />
        </a>
        @endif
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-5 relative z-10">
        ...
        <a href="{{ route('verbs.show', $fav->slug ?? '') }}" wire:navigate
            ...
            <div class="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span
                    class="bg-primary/10 text-primary text-[8px] font-bold px-2 py-1 rounded-full uppercase tracking-widest">
                    {{ $stats['percentage'] }}%
                </span>
            </div>
            ...
            <div>
                <h4
                    class="text-xl font-bold text-body group-hover:text-primary transition-colors tracking-tighter uppercase leading-tight mb-4">
                    {{ $fav->infinitive }}
                </h4>

                <div class="flex flex-wrap gap-1.5 opacity-60 group-hover:opacity-100 transition-opacity">
                    <span
                        class="text-[8px] font-bold text-muted uppercase tracking-widest">{{ $fav->past_simple }}</span>
                    <span
                        class="text-[8px] font-bold text-muted uppercase tracking-widest">{{ $fav->past_participle }}</span>
                </div>
            </div>
            ...
            <div class="mt-4 pt-4 border-t border-muted/30 flex items-center gap-3">
                ...
                <div class="flex -space-x-2">
                    ...
                    <div title="{{ $friend->username }}"
                        class="w-6 h-6 rounded-lg ring-2 ring-surface bg-indigo-500 flex items-center justify-center text-[8px] font-bold text-white shadow-sm">
                ...
                @else
                <span
                    class="text-[8px] font-bold text-muted uppercase tracking-widest italic group-hover:text-primary transition-colors">
        ...
        <div class="col-span-full text-center p-6 lg:py-12 bg-muted/5 rounded-xl border-2 border-dashed border-muted/30">
            ...
            <h4 class="text-sm font-bold text-body uppercase tracking-widest mb-2">Aucun favori pour le moment</h4>
